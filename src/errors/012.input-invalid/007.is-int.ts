/*
 * @Author:Kerwin
 * @Date:2024-04-09 17:49:54
 * LastEditors: Mike Huang
 * LastEditTime: 2025-01-15 11:30:04
 * @Description:
 */

import {
  GenerateErrorObject,
  GenerateErrorObjectWithTemplate
} from '$errors/base'

import {
  i18n
} from '$shared-service/i18n'

const errorTypeCode = '012.007'
const errorName = 'Input Invalid - is int'

export const errorParse = (code: ErrorCodes, message: string, field: string) =>
  GenerateErrorObject('INPUT_INVALID', `${errorTypeCode}.${String(code).padStart(3, '0')}`, errorName, message, { field })

export enum ErrorCodes {
  start = 1,
  end = 2,
  serviceTime = 3,
  priority = 4,
  startTime = 5,
  endTime = 6,
  maxDrivingDistance = 7,
  regularWorkTime = 8,
  maxDrivingTime = 9,
  counts = 10,
  UnknownError = 999
}

const generateInputInvalidError = GenerateErrorObjectWithTemplate(ErrorCodes, errorParse, i18n.t('errors:012.007.defaultMessage'))

export const isInt = {
  ...generateInputInvalidError
}

export default isInt
